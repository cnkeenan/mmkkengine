...
typedef void fp_glBegin (GLenum mode);
...
typedef void fp_glVertex2f(float x, float y);
...
#define FUNCTION_SIGNATURE(x) fp_##x* x
...
#define LOAD_GL_FUNCTION(x)                     \
    do                                          \
    {                                           \
        OpenGL->x = (fp_##x*)FunctionLoader(#x);        \
    }                                           \
    while(0)
